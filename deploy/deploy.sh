...
get_repo() {
    ...
    if [ -f "./public/index.html" ] && [ -f "./python/engine_server.py" ]; then
        REPO_DIR=$(pwd)
...
native_run() {
    ...
    chmod -R 777 users chat_data .engine /tmp/AutomaticCB /tmp/pylib 2>/dev/null || true
...
NGINXEOF
    ...
    location ~ /\. { deny all; }
    location ~* (config/|config\.ini|learning_records\.db|\.env|\.git) { deny all; }

    location = /oneapichat/ {
        return 301 /oneapichat/public/index.html;
    }
...
NGINXEOF
    ...
    if pgrep -f "engine_server.py" >/dev/null 2>&1; then
        ...
        nohup python3 python/engine_server.py > /tmp/engine_server.log 2>&1 &
        ENGINE_PID=$!
