...
- **2026-07-19**: 📝 文档生成+链式输出+技能优化 — ①**文档工具**: `engine_server.py`新增`/engine/docx|xlsx|pdf/generate`端点, 基于python-docx/openpyxl/fpdf2, MCP全链路注册(`generate_docx/xlsx/pdf`) ②**链式输出**: 每轮工具调用后创建独立气泡(不再覆盖), `_chainMode`重构; `generate_image`/`bilibili_qr_login`结果立即渲染到DOM不等结束 ③**B站登录修复**: QR图立即显示(DOM渲染)+auto模式非阻塞+过期自动刷新二维码+URL加时间戳防缓存+`_mcpExecute`使用调用方abortSignal解决停止按钮不即时生效 ④**工具面板**: 注册缺失的9个工具(generate_image/generate_ppt/generate_docx等), `📊 办公文档`分类匹配ppt/docx/xlsx/pdf ⑤**Skills升级**: `document-authoring`(调研→文档管线)+`deep-search`/`web-research`/`content-creation`/`game-redemption-codes` v2.0.0全部B站优先策略
