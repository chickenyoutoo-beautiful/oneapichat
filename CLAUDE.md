- **2026-09-07**: ⚡ **超星视频两倍速播放失效根除与用户配置优先恢复** — 彻底根除「在刷课与题库设置中配置了 2 倍速，但运行日志始终提示“课程不允许倍速，已自动使用 1 倍速”并被强制降为 1 倍速」：①**根因定位**: 任务卡属性 `property.doublespeed` 仅为超星 Web 播放器 UI 是否展示倍速按钮的前端标记（几乎所有课程默认均为 `doublespeed: 0`），超星后端多媒体日志上报接口（`/multimedia/log/a/...`）本身完全支持 2 倍速心跳推进；旧代码在 `study_video` 中硬编码了 `if _speed > 1 and _job.get('doublespeed') in (0, False): _speed = 1.0` 强行拦截，直接抹杀了用户在界面和配置中显式指定的 2 倍速参数；②**代码级恢复与日志透明化**: `python/chaoxing/base.py` 彻底移除对 `doublespeed` 的武断降速逻辑，严格遵循用户在 `config.ini`（`speed = 2`）中配置的倍速推进，并明确打印 `开始任务(2.0倍速): ...`；③编译刷新 Python 字节码，当前后台运行中的主任务保持零中断。

- **2026-09-07**: 🔄 **超星账号切换假登录、任务为零秒退与自动答题旧书签清理** — 彻底根除「切换超星账号识别到课程但点击刷课瞬间完成任务为零、再次尝试显示用户名密码错误、再次登录才正常」：①**旧 Cookie 盲目复用与假登录根除**: `python/chaoxing/api_get_courses.py` 增加 `--force-login` 参数，登录验证时跳过已有 Cookie 读取，强制执行 `chaoxing.login()` 走超星真实鉴权，杜绝复用旧 Cookie 导致密码错误却虚假判定登录成功的严重漏洞；②**账号切换与状态彻底深度清理**: `api/chaoxing_api.php` 的 `action=login` 在切换或重新登录时，彻底删除旧账号 Cookie 文件（`cookies_{userId}.pkl` 等）、课程缓存文件与旧 `course_list` 配置，彻底消除“旧账号课程 ID 残留导致新账号在 `main.py` 过滤后任务数量为零瞬间退出”的严重缺陷；③**登录接口 POST 现代化改造**: 前端 `public/chaoxing.html` 与后端统一改用 POST + JSON 传输密码，解决特殊字符、`+` 转空格与 URL Query 截断问题；登录成功后立即清空密码、清空已选计数并带 `force=true` 强制回源；④**下线无效浮动书签**: 彻底移除 `public/chaoxing.html` 中硬编码旧公网 IP 且已失效 404 的 `_bookmark_floating` 浮动自动答题书签组件；⑤全链路优化过程保持原有后台运行中的刷课进程零中断。

- **2026-09-07**: 🎓 **超星刷课配置缺失报错与 500 服务器内部错误彻底根除** — 根治刷课页面红字提示「配置文件缺失 [common] 节: /tmp/AutomaticCB/config_u_*.ini (文件可能为空或损坏)」与控制台 `api/chaoxing_api.php` 500 报错：①**PHP 端配置自愈与降级容错**: `api/chaoxing_api.php` 的 `case 'courses'` 补齐缺失的 `ensureUserConfig($userId)` 并在执行 Python 时显式透传 `--config`；`ensureUserConfig` 与 `saveUserConfig` 建立弹性容错，若写 `/tmp` 镜像受阻时静默降级返回持久化配置（`users/chaoxing/config_{userId}.ini`），绝不抛 `RuntimeException` 把接口打死成 500；`atomicWritePrivateFile` 升级权限为 `2775`/`0664` 并赋予 `www-data` 组权限，提供跨目录临时文件与 copy+unlink 兜底；②**pyCmd 运行目录与执行容错**: `api/init.php` 为 `pyCmd` 与 `pyBgCmd` 增加 `CHAOXING_DIR` 目录自动创建与 `cd` 失败回退（`cd CHAOXING_DIR 2>/dev/null || cd APP_ROOT`），根除 `can't cd to /tmp/AutomaticCB` 导致的退出码 2；③**Python 侧权威配置寻址与缓存自愈**: `python/chaoxing/config.py` 新增统一 `resolve_user_config`，优先寻址持久化目录 `users/chaoxing/`，彻底免疫系统重启和 `/tmp` 清空；`api_get_courses.py` 与 `exam_api.py` 接入统一配置解析并自动自愈运行时镜像；`answer.py` 为 `CacheDAO` 增加多目录可写探测与空安全防御；④**系统权限自愈加固**: 更新 `chaoxing-db-perms.service` 与 root crontab，将 `/tmp/AutomaticCB` 完整纳入每 2 分钟组权限自愈（`chown -R :www-data` + `chmod 2775`），经冷启动清空 `/tmp` 模拟测试与全链路 HTTP 验收 100% 恢复通过。

- **2026-09-06**: 🫧 **多轮工具调用气泡多重裂变与前置正文截断破碎彻底收敛** — 锁定并彻底解决「一次提问经历多轮 web_search 搜索在界面上依次吐出 3~4 个独立残缺 AI 气泡、前几个气泡说到一半被工具打断」：①**前端历史渲染同轮深度融合**: `dialogs.js`（`loadChat`）在构建 `displayMsgs` 时加入同轮连续 assistant 消息智能聚合算法，当连续多条 assistant 属于同一次提问迭代时，就地聚合成单个完整回复气泡；合并所有 `tool_calls` 徽章，通过增量与重叠文本算法平滑融合正文与思考，状态/用量/耗时自动更新为最新；②**废除实验性链式输出（Chain Mode）裂变**: 废弃容易导致界面破碎的链式气泡裂变模式，默认强制单气泡完整输出；③新增回归测试 `tests/bubble_fission_convergence.test.js` 并刷新前端版本构建。

- **2026-09-06**: 🌐 **WebSearch 工具前置抢跑输出与结果打断根治** — 彻底根除「web_search 还没运行完模型就直接输出，导致被工具的输出打断」：①**工具 Schema 调用纪律与原子性约束**: 在 `SEARCH_TOOL_DEFINITION` 中注入硬性约束：调用 `web_search` 工具时严禁在正文中预先输出任何回答、猜测或寒暄预热（如“我为您搜索”、“正在为您查询”等），必须直接且仅发起工具调用，等待工具返回完整搜索结果后在下一轮中综合给出最终回答；②**全局系统提示词约束注入**: `main.js` 在开启搜索或工具时，向系统提示词注入《联网搜索与工具调用规范》，严格指引模型在调用外部检索工具时保持正文静默，杜绝半成品输出；③通过 `tests/web_search_preamble_continuation.test.js` 与 `tests/tool_output_visibility.test.js` 全套断言测试，刷新资源版本。

- **2026-09-06**: 🛠️ **工具输出结果回传断流与模型重复调用死循环根治** — 彻底根治「模型调用工具后好像根本看不到工具输出结果，一直不断调用同一个工具陷入死循环」：①**Assistant 消息空正文豁免与规范化**: 锁定大模型在发起 `tool_calls` 时正文通常为空的工业标准规范，重构 `api-messages.js` 的消息安全过滤器，严禁对带有 `tool_calls` 的 assistant 消息盲目执行空 content 跳过，统一规范保留 `content: null`（完全符合 OpenAI / Gemini / DeepSeek 官方规范），消除 assistant 轮次被误杀抛弃；②**工具结果被连带误删与孤儿剔除根治**: 阻断 assistant 缺失引发 `normalizeToolMessagePairs` 误判孤立 tool 消息连带清空的恶性连锁反应；工具消息统一兜底，对空输出强制兜底为 `'(empty)'` 且必须为 string，去重方向统一为从前往后，并在严格邻接失败时引入宽松包含兜底，确保工具结果 100% 传回模型；③**全链路空结果兜底与测试覆盖**: `main.js` 同步补齐工具执行结果与发送前 `_mFix` 空内容防御；新增专用回归测试 `tests/tool_output_visibility.test.js` 覆盖纯工具调用、空结果工具、多轮连续工具调用全场景并通过断言，经 `build-index.py` 刷新前端构建。

- **2026-09-06**: 🧹 **陈旧队列自动误发、跨对话消息污染与空草稿夺舍彻底根除** — 彻底根除「从老设备登录或访问时以前发送过的消息莫名其妙自动发送」以及「在另一个对话发送的消息串进新对话」：①**队列生命周期收敛与防自动误发**: `queue.js` 为持久化队列固化 `savedAt`、`userId` 与 `chatId` 三重维度校验，实施 30 秒严格 TTL 熔断；`dialogs.js` 彻底移除 `loadChat` 切换会话时 `setTimeout(window._drainQueue, 500)` 的盲目自发逻辑；`_drainQueue` 增加当前会话强隔离守卫，绝不向非当前会话发送或串入当前会话；②**多设备同步空草稿防夺舍闭环**: `agent-notify.js` 彻底根除 `chat:updated` 与 `chat:stream_started` 中因检测到当前为空草稿而仅篡改 `currentChatId = cid` 的静默偷换缺陷，严禁后台事件劫持当前用户的空白新对话与输入状态，彻底阻断其他会话的实时流与历史消息误灌入新对话；③**输入框草稿结构化隔离与冷启动脏缓存清洗**: `init.js` 在页面启动首帧通过 `_purgeStaleTransientStorage` 清空全部遗留队列、残留流标记与过期快照；`_savedInputText` 增加 `chatId`、`userId` 与 60 秒时效校验，跨会话/跨用户直接废弃；新增回归测试 `tests/stale_cache_and_queue_pollution.test.js` 并刷新前端构建。

- **2026-09-06**: 🧭 **管理后台与 Mihomo 节点管理返回死循环与来源丢失根治** — 彻底解决从 OneAPIChat 聊天页进入管理后台后打开 Mihomo 节点管理，返回管理后台时返回按钮变成“← 返回节点”导致无法返回聊天的致命死循环：①**消除逻辑颠倒与子页面覆盖**: 根除 `manager.html` 中错误的 `fromParam === 'mihomo'` 分支（子工具页面不能反客为主把主管理面板的返回按钮劫持为“返回节点”）；②**来源持久化与子工具过滤**: `manager.html` 引入 `sessionStorage` 来源状态机，在进入时精准锁定 `chat` 或 `home` 原始入口，排除子工具 `/public/mihomo` 对 `document.referrer` 的污染并动态透传 `from` 给子工具卡片；③**Mihomo 返回安全闭环**: `public/mihomo.html` 彻底移除硬编码的 `?from=mihomo`，升级为浏览器历史栈优先返回（`history.back()`）+ 动态带源兜底跳转（`goBackManager`）；新增全链路行为回归测试 `tests/manager_mihomo_return_flow.test.js` 并通过全套断言。
