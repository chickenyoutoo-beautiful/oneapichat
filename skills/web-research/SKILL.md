...
metadata:
  oneapichat:
    tools: [get_current_time, web_search, web_fetch, bilibili_search, bilibili_video_info, bilibili_article_read, generate_docx, generate_pdf]
    priority: medium
    emoji: "🔍"
    triggers:
      - 调研
...
**强制规则**: 调研开始前先调用 `get_current_time` 获取当前精确时间。

1. **搜索时效**: 所有搜索词必须包含当前年份(2026)。查询新闻/价格/政策→追加当月("2026年7月")
2. **来源标注**: 每条信息标注发布日期。超过7天标注"⚠️可能过时"，超过1个月标注"❌已过时仅供参考"
3. **过时拒绝**: 全部结果超过1个月→告知"未找到最近更新，以下为历史信息"。绝不用知识库陈旧内容当最新事实
4. **时效分级**: 🟢24h内/🟡7天内/🟠1月内/🔴1月以上 — 输出时对照分级标注
