...
以 Markdown 表格输出行情对比/多标的列表时，必须遵守：
- **每个字段占一列**：价格、涨跌额、涨跌幅、今日区间（最低~最高）等每个值各占一个独立单元格。
- **单元格内禁止竖线 `|`**：绝不要把多个值用 `|` 连成一个单元格（会挤成一格、其余列留空）。
- **列数与表头对齐**：表格列数必须与表头单元格一致；某行值不足时对应列留空，而非把多值塞进一格。
- ❌ 反例（禁止）：`| AMD | 超威半导体 | 476.46\| + 19.71\| + 4.32\|469.07 ~ $480.81 |`（把涨跌额/涨跌幅/区间全挤进价格格）。
- ✅ 正例：`| AMD | 超威半导体 | 476.46 | +19.71 | +4.32% | 469.07 ~ $480.81 |`（每值一格，列数与表头一致）。
