...
window.pushToMsgQueue = function() {
    if (!isAgentToolsActive()) return;
    var input = $.userInput;
    var text = input ? input.value.trim() : '';
    if (!text && (!pendingFiles || pendingFiles.length === 0)) return;
    
    window._messageQueue.push({
        id: ++window._queueIdCounter,
        text: text,
        files: pendingFiles ? [].concat(pendingFiles) : []
    });
...
/** 排干队列 — 只在下一条消息且 AI 空闲时调用 */
window._drainQueue = async function() {
    if (!isAgentToolsActive()) { window._messageQueue = []; window._isQueueProcessing = false; window._updateQueueUI(); return; }
    if (window._isQueueProcessing) return;  // 正在处理中,等完成
    if (window._messageQueue.length === 0) {
        window._isQueueProcessing = false; window._updateQueueUI(); return;
    }
    if (isTypingMap[currentChatId]) {
        // AI 正忙:等 sendMessage 的 finally 中调用
        return;
    }
    ...
    // 标记为队列消息,让 sendMessage 跳过 Task 创建
    window._isQueueMessage = true;
    try {
        await window.sendMessage(false, item.text);
    } catch(e) {
...
window._updateQueueUI = function() {
    ...
    var qBar = getEl('queueBar');
    var qText = getEl('queueBarText');
    var qBadge = getEl('queueBarBadge');
    var qCount = window._messageQueue.length;
    
    // AI生成中且队列不为空时显示队列栏
    var aiBusy = isTypingMap[currentChatId];
    var showBar = aiBusy && qCount > 0;
    if (qBar) qBar.classList.toggle('hidden', !showBar);
    
    if (qText) {
        if (qCount === 0) qText.textContent = '消息队列';
        else if (qCount === 1) qText.textContent = '1 条消息排队中';
        else qText.textContent = qCount + ' 条消息排队中';
    }
    if (qBadge) {
...
window.sendMessage = async function (skipUserAdd, userTextForRegen, userFilesForRegen) {
    ...
    if (isTypingMap[chatId]) {
        ...
        if (!skipUserAdd && userTextForRegen && userTextForRegen.length > 0 && isAgentToolsActive()) {
            window._messageQueue.push({ text: userTextForRegen, files: pendingFiles ? [].concat(pendingFiles) : [] });
            pendingFiles = [];
            window._updateQueueUI();
