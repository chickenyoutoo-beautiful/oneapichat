...
def decode_course_point(_text):
    ...
    for _chapter_unit in _soup.find_all("div", class_="chapter_unit"):
        ...
        for _point in _raw_points:
            _point = _point.div
            if (not _point or "id" not in _point.attrs):
                continue

            _point_detail = {}
            match = re.findall(r"^cur(\d{1,20})$", _point.attrs["id"])
            if not match:
                continue
            _point_detail["id"] = match[0]

            title_tag = _point.select_one("a.clicktitle")
            _point_detail["title"] = title_tag.text.replace("\n", '').strip(' ') if title_tag else "未知标题"

            _point_detail["jobCount"] = 1   # 默认为1
            job_tag = _point.select_one("input.knowledgeJobCount")
            if job_tag:
                _point_detail["jobCount"] = job_tag.attrs.get("value", 1)
            else:
                # 判断是不是因为需要解锁
                tip_tag = _point.select_one("span.bntHoverTips")
                if tip_tag and '解锁' in tip_tag.get_text():
