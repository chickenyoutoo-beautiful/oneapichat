...
function buildUserContent(text, files) {
    ...
    if (hasImages) {
        ...
        const otherFiles = files.filter(f => !f.type?.startsWith('image/') && !f.hasEmbeddedImages);
...
const _TOOL_CATEGORIES = [
    ...
    { label: '🤖 引擎/Agent', keys: ['ENGINE_CRON_LIST_TOOL','ENGINE_CRON_CREATE_TOOL','ENGINE_CRON_DELETE_TOOL','DELEGATE_TASK_TOOL','ENGINE_AGENT_STATUS_TOOL','ENGINE_AGENT_LIST_TOOL','ENGINE_AGENT_DELETE_TOOL','ENGINE_PUSH_TOOL','PLAN_UPDATE_TOOL'], agentOnly: true },
...
const _TOOL_LABELS = {
    ...
    'ENGINE_AGENT_DELETE_TOOL': '删除子代理', 'ENGINE_PUSH_TOOL': '推送通知', 'PLAN_UPDATE_TOOL': '计划更新',
...
window.fetchModels = async function (silent) {
    ...
    try {
        ...
        const _tid = setTimeout(() => _ctrl.abort(), 8000);  // 8s 超时
        const res = await window.proxyFetch(`${url}/models`, { headers: _headers, signal: _ctrl.signal });
        clearTimeout(_tid);
    ...
    if (role === 'user') {
        ...
        const editBtn = document.createElement('div');
            ...
            if (_isLastAsst) {
                ...
                restoreBtn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/><path d="M9 15h6"/><path d="M9 19h4"/></svg>';
...
window._saveQueue = function() {
    try {
        ...
        // ★ 队列为空时直接删除 key，不留空数组残留
        if (window._messageQueue.length === 0) {
            localStorage.removeItem(_key);
            console.log('[Queue] save: EMPTY, removed key=' + _key);
            return;
        }
        ...
        console.log('[Queue] save: ' + data.length + ' items to key=' + _key);
    } catch(e) {
...
window._loadQueue = function() {
    try {
        ...
        if (!Array.isArray(data)) { console.log('[Queue] load: invalid data for key=' + _key); return false; }
        window._messageQueue = data;
        var maxId = 0;
        data.forEach(function(item) { if (item.id > maxId) maxId = item.id; });
        window._queueIdCounter = maxId;
        console.log('[Queue] load: ' + data.length + ' items from key=' + _key);
        return true;
    } catch(e) {
...
window._drainQueue = async function() {
    ...
    // ★ 及时持久化：队列为空时立即清除，不等下次 _drainQueue
    if (window._messageQueue.length === 0) {
        window._clearPersistedQueue();
    } else {
        window._saveQueue();
    ...
    // ★ 后台推入：不跳转会话，使用 loadChat 静默加载目标聊天上下文
...
window.sendMessage = async function (skipUserAdd, userTextForRegen, userFilesForRegen) {
    if (!skipUserAdd && !window._isQueueMessage) {
        // ★ 清除 ask_agent 单次授权（每轮用户消息重新评估）
        if (window._tempAgentGranted) {
            delete window._tempAgentGranted;
            delete window._tempAgentChatId;
            _updateTempGrantBanner(false);
            console.log('[AskAgent] 单次授权已回收（新用户消息）');
        }
        // 用户发起的消息 → 新任务批次开始
        // ★ 创建任务,后续代理将关联到这个任务ID
    ...
    if (toolMode) {
            ...
            for (var _fti = 0; _fti < tools.length; _fti++) {
                ...
                if (_toggleKey) {
                    if (window.isToolEnabled(_toggleKey)) {
                        ...
                        var _agentOn = isAgentToolsActive();
                        var _agentOnlyKeys = ['SERVER_EXEC_TOOL','SERVER_PYTHON_TOOL','SERVER_FILE_WRITE_TOOL','SERVER_FILE_OP_TOOL','SERVER_DOCKER_TOOL','SERVER_DB_QUERY_TOOL','ENGINE_CRON_LIST_TOOL','ENGINE_CRON_CREATE_TOOL','ENGINE_CRON_DELETE_TOOL','DELEGATE_TASK_TOOL','ENGINE_AGENT_STATUS_TOOL','ENGINE_AGENT_LIST_TOOL','ENGINE_AGENT_DELETE_TOOL','ENGINE_PUSH_TOOL','PLAN_UPDATE_TOOL','BROWSER_NAVIGATE_TOOL','BROWSER_SCREENSHOT_TOOL','BROWSER_CLICK_TOOL','BROWSER_TYPE_TOOL','BROWSER_GET_CONTENT_TOOL','BROWSER_GET_SNAPSHOT_TOOL'];
        ...
        if (!_isInNoToolList) {
            ...
            if (!_useAnthropicFormat && (agentModeActive || !isMiniMaxModel)) body.tool_choice = "auto";
        } else {
...
// 执行每个工具调用并添加结果(只对有有效内容的tool call执行)
            ...
            // ★ 400 错误智能重试: 解析错误原因，尝试修复后重试
            const is400Error = e.message && (e.message.startsWith('HTTP 400') || e.message.includes('HTTP 400:'));
            if (is400Error && attempt < maxRetries) {
                var _errBody = '';
                var _errJson = null;
                try {
                    _errBody = e.message.replace(/^HTTP 400:\s*/, '');
                ...
                } else if (_errMsg.includes('tool') && (_errMsg.includes('not support') || _errMsg.includes('disabled'))) {
                ...
                if (_shouldRetry) {
                    ...
                    if (_retryAction === 'trim_context') {
                        ...
                        showToast('⚠️ 上下文过长，已自动裁剪消息历史后重试...', 'warning', 3000);
                        ...
                        showToast('⚠️ max_tokens 超限，已调整为 ' + (body.max_completion_tokens || body.max_tokens) + ' 后重试...', 'warning', 3000);
                        ...
                        showToast('⚠️ System prompt 过长，已截断后重试...', 'warning', 3000);
                        ...
                        showToast('⚠️ 工具调用参数异常，已自动修复后重试...', 'warning', 3000);
                        ...
                        showToast('⚠️ 工具参数异常，已切换为普通模式重试...', 'warning', 3000);
                        ...
                        delete body.stop;
                        showToast('⚠️ 参数异常，已清理后重试...', 'warning', 3000);
                    } else {
                        // generic_retry: 只等一会重试
                        showToast('⚠️ 请求异常(400)，' + (attempt + 1) + '/' + maxRetries + ' 重试中...', 'warning', 3000);
                    }
...
var RAG_API = '/oneapichat/api/rag_proxy.php';
...
window._wsInit = function() {
    if (localStorage.getItem('__enableResumeStream') !== '1') {
        console.log('[WS] Toggle OFF, skipping WebSocket');
        return;
    }
    console.log('[WS] Toggle ON, connecting WebSocket...');
    // 恢复上次的流状态
    try {
...
window.appendAgentSystemMessage = function(text, source) {
    if (!text) return;
    // ★ 推送消息渲染为居中通知卡片（DOM 直接注入，不走 markdown 渲染）
    if (currentChatId) {
        var _src = source || '子代理';
        var _time = new Date().toLocaleTimeString('zh-CN', {hour:'2-digit',minute:'2-digit',second:'2-digit'});
        var _card = document.createElement('div');
        _card.className = 'agent-push-card';
        _card.innerHTML = '<div class="agent-push-header">' +
            ...
            '<span class="agent-push-source">' + escapeHtml(_src) + '</span>' +
            '<span class="agent-push-time">' + _time + '</span>' +
        '</div>' +
        '<div class="agent-push-body">' + escapeHtml(text).replace(/\n/g, '<br>') + '</div>';
        var _container = document.getElementById('chatMessagesContainer');
        if (_container) {
            _container.appendChild(_card);
            setTimeout(function() { autoScrollToBottom('push'); }, 50);
        }
    }
