...
window.onSearchProviderChange = async function() {
    ...
    // 1. 保存当前 Key 到旧引擎
    const curKey = getVal('searchApiKey') || '';
    const oldProvider = localStorage.getItem('searchProvider') || 'duckduckgo';
    if (oldProvider && oldProvider !== provider && curKey) {
        const oldKeyId = SEARCH_PROVIDER_KEY_MAP[oldProvider];
        if (oldKeyId) localStorage.setItem(oldKeyId, await encrypt(curKey));
    }
    // 2. 切换到新引擎的 Key (优先独立 Key,其次通用 Key)
    const newKeyId = SEARCH_PROVIDER_KEY_MAP[provider];
    var savedProviderKey = newKeyId ? localStorage.getItem(newKeyId) : null;
    if (newKeyId && savedProviderKey) {
        const dk = await decrypt(savedProviderKey);
...
async function loadSearchConfig() {
    ...
    const ragChecked = localStorage.getItem('ragEnabled') !== 'false';
    ...
    const _proxyDetails = document.getElementById('proxyConfigDetails');
...
const usageVisualizer = {
  ...
  /** 缓存命中提示 */
  cacheHint: function() {
    const totalCache = sessionUsage.cacheHitTokens + sessionUsage.cacheMissTokens;
    if (totalCache === 0) return '';
    const rate = (sessionUsage.cacheHitTokens / totalCache * 100).toFixed(1);
  ...
  toolStatsDisplay: function() {
    const top = toolCallStats.getTopTools(5);
...
window.renderToolPanel = function() {
    ...
    if (!container) return;
    const existingRows = container.querySelectorAll('.tool-toggle-row.dynamic, .tools-category-label.dynamic');
    ...
    _cats.forEach(function(cat) {
        const _disabled = cat.agentOnly && !_agentOn;
        if (_disabled) {
        ...
        cat.keys.forEach(function(key) {
            ...
            const label = (typeof _TOOL_LABELS !== 'undefined' ? _TOOL_LABELS[key] : null) || key.replace(/_/g, ' ');
            const isDanger = (key.indexOf('SERVER_EXEC') >= 0 || key.indexOf('SERVER_PYTHON') >= 0 || key.indexOf('SERVER_FILE_WRITE') >= 0 || key.indexOf('SERVER_DOCKER') >= 0 || key.indexOf('SERVER_DB') >= 0 || key.indexOf('SERVER_FILE_OP') >= 0 || key.indexOf('CRON_CREATE') >= 0 || key.indexOf('CRON_DELETE') >= 0 || key.indexOf('AGENT_DELETE') >= 0);
            const warnClass = isDanger ? ' tool-warn' : '';
            const checked = window.isToolEnabled(key) ? ' checked' : '';
            const disabledAttr = _disabled ? ' disabled' : '';
...
window.updateToolsActiveCount = function() {
    ...
    // 加上自定义技能
    const customSkills = window.getCustomSkills();
...
window.renderCustomSkillsList = function() {
    ...
    container.querySelectorAll('[data-custom-skill]').forEach(function(el) {
        el.addEventListener('change', function() {
            const skillName = this.getAttribute('data-custom-skill');
            if (skillName) {
...
window.generateSkillDefinition = async function() {
    const desc = document.getElementById('skillDescriptionInput').value.trim();
    if (!desc) {
    ...
    // 检测当前模型是否支持工具调用
    const currentModel = getVal('modelSelect') || 'deepseek-v4-flash';
    var isNoTool = false;
    try {
        const noToolModels = JSON.parse(localStorage.getItem('noToolModels') || '[]');
    ...
    const systemPrompt = '你是一个工具定义生成器。根据用户的描述,生成一个符合 OpenAI function calling 格式的 tool definition JSON。\n\n' +
    ...
    try {
        const resp = await window.proxyFetch(baseUrl + '/chat/completions', {
            method: 'POST',
            headers: {
...
window.saveCustomSkill = function() {
    const jsonStr = document.getElementById('skillDefinitionPreview').value.trim();
    if (!jsonStr) {
...
async function saveConfig(showFeedback = false) {
    ...
    try {
        ...
        const _pCfg = API_PROVIDERS[_provider] || API_PROVIDERS.custom;
    ...
    const _sak = getVal('searchApiKey') || '';
...
window.toggleProxy = function() {
    ...
    localStorage.setItem('proxyUrl', getVal('proxyUrl') || '');
    const details = document.getElementById('proxyConfigDetails');
...
function _updateThinkingVisibility() {
    const _tl = getEl('thinkingModeRow');
    if (!_tl) return;
    const _m = (getVal('modelSelect') || '').toLowerCase();
    const _bu = (getVal('baseUrl') || '').toLowerCase();
...
window.proxyFetch = async function(targetUrl, options = {}) {
    const proxyUrl = window.getProxyUrl();
...
window.fetchModels = async function (silent) {
    ...
    var _provider = getEl('baseUrlProvider')?.value || 'custom';
    const _isLocalModel = _provider === 'llamacpp';
    ...
    try {
        const _headers = _isLocalModel ? {} : { Authorization: `Bearer ${key}` };
        ...
        if (mainSelect) {
            ...
            const _p = getEl('baseUrlProvider')?.value || 'custom';
            const _storedModel = localStorage.getItem('model_' + _p) || localStorage.getItem('model') || '';
            ...
            if (!mainSelect._modelChangeBound) {
                ...
                mainSelect.addEventListener('change', function() {
                    ...
                    const _p2 = getEl('baseUrlProvider')?.value || 'custom';
        ...
        ['titleModel', 'searchModel', 'aiSearchJudgeModel'].forEach(id => {
            const sel = getEl(id);
            if (!sel) return;
            const curMainModel = getVal('modelSelect') || 'deepseek-v4-flash';
            const mainLabel = models.find(m => m.id === curMainModel)?.label || curMainModel;
            const placeholder = '<option value="">同主模型 (' + mainLabel + ')</option>'
        ...
        if (curModel && modelContextLength[curModel]) {
            const ctxMax = modelContextLength[curModel] - MAX_TOKENS_SAFETY_MARGIN;
            const outMax = modelMaxOutputTokens[curModel] || ctxMax;
            const max = Math.min(ctxMax, outMax);
            // ★ 完全按用户配置,不按模型调整
...
window.refreshModels = async function (e) {
    ...
    try {
        ...
        const _em = (e && e.message) ? e.message : '';
        ...
        // ★ 确保旋转动画至少显示了 600ms
        const _elapsed = Date.now() - _spinStart;
        const _minDelay = Math.max(0, 600 - _elapsed);
        setTimeout(function() {
            if (btn) {
                btn.disabled = false;
