...
async function extractFileContent(file) {
    ...
    if (ext === 'docx' || file.type.includes('word')) {
        ...
        if (window.mammoth) {
            try {
                const _result = await mammoth.extractRawText({ arrayBuffer: _ab });
        ...
        if (!_docText) {
            const _raw = new TextDecoder('utf-8', {fatal: false}).decode(_ab);
            const _texts = [];
            const _re = /<w:t[^>]*>([^<]*)<\/w:t>/g; let _m;
        ...
        if (window.JSZip) {
            try {
                const _docZip = await JSZip.loadAsync(_ab);
    ...
    if (ext === 'pptx' || ext === 'ppt') {
        ...
        try {
            ...
            const _raw2 = new TextDecoder('utf-8', {fatal: false}).decode(arrayBuf);
            ...
            // 3) 完全无法提取 — 明确告知用户
            const _errDetail = zipErr.message || '';
        ...
        for (let i = 0; i < slideFiles.length; i++) {
            ...
            if (slideText.trim()) {
                ...
                const slideEntry = '【幻灯片 ' + (i + 1) + '】' + slideText;
...
window._previewUploadedFile = function(index) {
    ...
    if (isImg) {
        ...
        const isText = f.name && /\.(txt|md|json|js|ts|jsx|tsx|html|css|xml|yaml|yml|py|php|go|rs|sh|log|conf|ini|cfg|csv|sql|env)$/i.test(f.name);
    ...
    for (const file of Array.from(fileList)) {
        ...
        try {
            if (isImage) {
                ...
                const rawDataUrl = 'data:' + file.type + ';base64,' + base64;
                ...
                const compressedBytes = atob(dataUrl.split(',')[1] || '').length;
                ...
                const _compType = (dataUrl.match(/^data:(image\/[\w+]+);/) || [])[1] || 'image/jpeg';
                ...
                _setProgress(20, '解析中...');
                const _extractResult = await extractFileContent(file);
                // ★ 支持 office 文档返回 {text, images, isOfficeDoc} 对象
                if (_extractResult && typeof _extractResult === 'object' && _extractResult.isOfficeDoc) {
                    const _fileObj = { name: file.name, content: _extractResult.text || '', size: file.size, isImage: false, type: file.type };
